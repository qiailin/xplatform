...
public class AllUserDaoImpl extends BaseDaoImpl implements IAllUserDao {
	...
	public List<AllUsers> getAllUsersByOrgId(Long orgId) {
		return getSqlMapClientTemplate().queryForList(
				"alluser.getAllUsersByOrgId", orgId);
	}
	...
	public int getUserChangeCount(UserChange bUser) {
		return (Integer) getSqlMapClientTemplate().queryForObject(
				"alluser.getUserChangeCount", bUser);
	}

	@SuppressWarnings("unchecked")
	public List<UserChange> getUserChangeList(UserChange bUser) {
		return getSqlMapClientTemplate().queryForList(
				"alluser.getUserChangeList", bUser);
	}

	public int modifyAllUserInfo(AllUsers allUsers) {
		return getSqlMapClientTemplate().update("alluser.modifyAllUser",
				allUsers);
	}

	public int deleteBImplementByEmpId(Long userId) {
		return getSqlMapClientTemplate().delete(
				"alluser.deleteBImplementByEmpId", userId);
	}

	public int searchUserChangeCount(UserChange bUser) {
		return (Integer) getSqlMapClientTemplate().queryForObject(
				"alluser.searchUserChangeCount", bUser);
	}

	public Long insertUserChange(UserChange bUser) {
		return (Long) getSqlMapClientTemplate().insert(
				"alluser.insertUserChange", bUser);
	}

	public int getCheckUserCount(ApplyUsers applyUsers) {
		return (Integer) getSqlMapClientTemplate().queryForObject(
				"alluser.getCheckUserCount", applyUsers);
	}

	@SuppressWarnings("unchecked")
	public List<ApplyUsers> getCheckUserList(ApplyUsers applyUsers) {
		return getSqlMapClientTemplate().queryForList(
				"alluser.getCheckUserList", applyUsers);
	}
